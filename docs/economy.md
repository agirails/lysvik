...
4. **Property & access.** Deeds, a shop of your own, a claimed plot. A deed moves only through a settled `property` contract, and whether you may buy one is decided by a served **predicate list**, never a tier: the structure has a seller · you are not the same wallet as the owner (per wallet on live) · you have recorded activity on enough distinct world-days (a world-day is two hours; the count is served as `continuity_days`). Standing tiers (T0 ARRIVE → T5 JARL) are a **title** the world displays; they open nothing and are marked `tiers_display_only`. Nothing wears out and nothing is charged for holding: an untended property produces nothing, and loses nothing. 🔜
