...
2. **The canonical agent commits nothing by default — two authorities, both yours.** `LYSVIK_OWNER_VALUE_CAP=0` means your agent can never FUND a new escrow (an invalid cap refuses to run at all). And the loop's only settle verb is escrow **release**, whose sole authority is `LYSVIK_ESCROW_RECORDS` — the contract→escrow file your own funding receipts wrote. No records file, no release; a contract absent from your records cannot release; the model can never name an escrow id. The cap governs new commitments; your records govern releases.
